...
#   about, above, across, after, against, along, amid, among, around, at, before, behind, below, beneath, beside, besides, between, beyond, by, concerning, considering, despite, down, during, except, for, from, in, inside, into, like, near, of, off, on, onto, opposite, out, outside, over, past, per, regarding, round, since, than, through, throughout, till, to, toward, towards, under, underneath, unlike, until, up, upon, via, with, within, without
#   according to, adjacent to, ahead of, along with, apart from, as for, as of, as per, as to, aside from, away from, because of, close to, due to, except for, far from, in addition to, in case of, in front of, in lieu of, in place of, in spite of, instead of, next to, on account of, on behalf of, on top of, out of, outside of, owing to, prior to, regardless of, subsequent to, together with, up to
#   as a result of, at the expense of, by means of, by virtue of, by way of, in accordance with, in back of, in comparison with, in contrast to, in keeping with, in light of, in order to, in place of, in reference to, in regard to, in relation to, in respect of, in terms of, in the event of, in the face of, in view of, on the basis of, on the part of, on the side of, with reference to, with regard to, with respect to, with the exception of
    ...
    unless has_toc_right
      if has_toc
        ...
        if first_heading_idx
          text = text.insert(first_heading_idx, "{{TOC right}}\n\n")
...
def get_glossary_terms(wiki)
  ...
  tables.each_with_index do |table_content, table_idx|
    ...
    rows.each do |row|
      ...
      if cells.length >= 2
        term = cells[0].gsub(/''+'(.+?)''+/, '\1').strip  # Remove bold/italic markup
  ...
  status("Glossary", "loaded #{glossary_terms.length} terms", false, :light) if glossary_terms.length > 0
  glossary_terms
end
...
def apply_glossary_tags(text, glossary_terms)
  ...
  glossary_terms.each do |term, definition|
    ...
    abbr_pattern = /<abbr[^>]*>#{escaped_term}<\/abbr>/
    ...
    if first_is_abbr
      ...
      if abbr_full.match?(/title="([^"]*)"/)
        ...
        if current_title != definition
          updated_abbr = abbr_full.gsub(/title="[^"]*"/, %Q{title="#{definition}"})
      ...
      while true
        ...
        text[search_start...(search_start + next_match.end(0))] = text[search_start...(search_start + next_match.end(0))].sub(next_abbr_full, next_unwrapped)
        terms_removed += 1
      ...
      replacement = %Q{<abbr title="#{definition}">#{match_text}</abbr>}
      text = text[0...match_idx] + replacement + text[(match_idx + match_text.length)..-1]
      terms_tagged += 1
      status("glossary_tagged_first", "#{term}: #{definition.truncate(40)}", false, :verbose)
      
      # Remove abbr tags from any other instances of this term
      unwrapped_abbr_pattern = /<abbr[^>]*>#{Regexp.escape(match_text)}<\/abbr>/
  ...
  status("glossary_summary", "found:#{terms_found} matched:#{terms_matched} updated:#{terms_updated} tagged:#{terms_tagged} removed:#{terms_removed}", false, :verbose)
  text
end
...
def enforce_word_boundaries(rules)
  rules.each do |rule|
    ...
    if find_pattern.match?(/^[a-zA-Z0-9_\-']+$/)
...
def convert_html_lists_to_wikitext(text)
  ...
  text.gsub!(/<\/?p[^>]*>/i, '')
  text.gsub!(/<(ul|ol|dl)[^>]*>/i) { "<#{$1.downcase}>" }
  text.gsub!(/<(li|dt|dd)[^>]*>/i) { "<#{$1.downcase}>" }
  ...
  tag_regex = /(<ul?>|<\/ul?>|<ol?>|<\/ol?>|<dl?>|<\/dl?>|<li>|<\/li>|<dt?>|<\/dt?>|<dd?>|<\/dd?>)/i
...
def categories_to_bottom(text)
  ...
  clean_text = protected_text.gsub(/___TEMPLATE_(\d+)___/) { placeholders[$1.to_i] }.strip
  clean_text.gsub!(/\n{3,}/, "\n\n")

  # 5. Format category block: one per line, trimmed of trailing spaces
  category_block = categories.map { |c| c.strip }.join("\n")

  applied_summaries << "MW Lint: Moved #{categories.length} categories to bottom of page" if categories.length > 0
...
rules_files.each do |rules_file_config|
  ...
  begin
    ...
    file_rules.each { |r| r['_source'] = rules_file_config[:name]; r['_dubious'] = rules_file_config[:dubious] }
...
status("rules_disabled", RULES_CONFIG.select { |_, v| !v }.keys.join(', '), false, :verbose) if RULES_CONFIG.any? { |_, v| !v }
  ...
  site_cfg['namespaces'].each do |ns|
    ...
    pages.each do |title|
      ...
      begin
        ...
        new_text, icon_changed = [current_text, false]
      ...
      if RULES_CONFIG[:mw_linting]
        ...
        begin
          ...
          new_text, toc_changed, heading_count, toc_status = [current_text, false, 0, nil]
      ...
      rules.each do |rule|
        ...
        if current_text.match?(find_reg)
...
total_pages_changed = report.values.sum { |pages| pages.length }
...
begin
  ...
  $stderr.puts "Error writing report #{report_name}: #{e.message}"
  ...
  begin
    ...
    $stderr.puts "Fallback report write failed: #{e2.message}"
